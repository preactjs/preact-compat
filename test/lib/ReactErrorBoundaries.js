...
describe('ReactErrorBoundaries', () => {
  ...
  it('does not swallow exceptions on mounting without boundaries', () => {
    expect(() => {
      ReactDOM.render(<BrokenRender />, scratch);
		}).throw('Hello');

		expect(() => {
      ReactDOM.render(<BrokenComponentWillMount />, scratch);
		}).throw('Hello');

    expect(() => {
      ReactDOM.render(<BrokenComponentDidMount />, scratch);
    }).throw('Hello');
  });
  ...
  it.only('prevents errors from leaking into other roots', () => {
